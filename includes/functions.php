...
function wd_ac_insert_address( $args = [] ) {
    ...
    if ( empty( $args['name'] ) ) {
        return new \WP_Error( 'no-name', __( 'You must provide a name.', 'ramLit-academy' ) );
    }
    ...
    if ( isset( $data['id'] ) ) {
        ...
        if ( ! $inserted ) {
            return new \WP_Error( 'failed-to-insert', __( 'Failed to insert data', 'ramlit-academy' ) );
        }
 ...
 * @return int|boolean
 */
function rm_ac_delete_address( $id ) {
    global $wpdb;
 ...
 * @return void
 */
function rm_ac_address_purge_cache( $book_id = null ) {
    $group = 'address';
